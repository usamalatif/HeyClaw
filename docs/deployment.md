...
docker exec -i YOUR_POSTGRES_CONTAINER psql -U postgres -c "CREATE USER openclaw WITH PASSWORD 'PICK_A_SECURE_PASSWORD';"
...
```bash
docker exec -i YOUR_POSTGRES_CONTAINER psql -U openclaw -d openclaw_app -c "\dt"
...
OPENCLAW_GATEWAY_TOKEN=PASTE_OUTPUT_OF_NEXT_COMMAND
